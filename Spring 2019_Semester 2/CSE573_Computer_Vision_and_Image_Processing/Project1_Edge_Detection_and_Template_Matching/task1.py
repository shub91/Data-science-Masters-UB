...
def convolve2d(img, kernel):
    ...
    img_conv = np.zeros((len(img), len(img[0]))) # Taking image with all zeros
    ker = utils.flip2d(kernel) # flipping the kernel
    pwx = len(kernel) // 2
    pwy = len(kernel[0]) // 2
    padded_img = utils.zero_pad(img, pwx, pwy) # Zero padding
    for i in range((len(padded_img) - len(ker) + 1)):
    	for j in range((len(padded_img[0]) - len(ker[0]) + 1)):
    		img_patch = utils.crop(padded_img, i, i + len(ker), j, j + len(ker[0]))
    		img_conv[i][j] = sum(map(sum, utils.elementwise_mul(img_patch, ker)))
...
def normalize(img):
    ...
    max_val = max([max(r) for r in img])
    min_val = min([min(r) for r in img])
...
def edge_magnitude(edge_x, edge_y):
    ...
    """
    # TODO: implement this function.
    x_edge2 = utils.elementwise_mul(edge_x, edge_x)
    y_edge2 = utils.elementwise_mul(edge_y, edge_y)
    F = np.sqrt(utils.elementwise_add(x_edge2, y_edge2))
    edge_mag = normalize(F)
    # raise NotImplementedError
    return edge_mag
