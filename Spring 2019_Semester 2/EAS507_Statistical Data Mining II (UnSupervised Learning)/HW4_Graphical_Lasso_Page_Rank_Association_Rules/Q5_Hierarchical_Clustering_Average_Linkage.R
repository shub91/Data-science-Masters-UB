...
## function for Average-linkage agglomerative hierarchical clustering
## Shubham Sharma                                                   
...
for (i in 1:(rownum-1)) { 
  ...
  colnames(D)[clus] = i
  r = 0.5*apply(D[d,], 2, min) + 0.5*apply(D[d,], 2, max)
  D[min(d),] = r
  D[,min(d)] = r
  D[min(d), min(d)] = Inf
  D[max(d),] = Inf
...
quartz()
plot(h, main = "Agglomerative Hierarchical Clustering using Average Linkage", cex = 0.5)
